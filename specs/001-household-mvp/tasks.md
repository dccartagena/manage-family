...
- [ ] T014 [P] Implement `web/src/lib/supabase.ts` — auth-only Supabase client (FR-023 headless: frontend uses Supabase SDK exclusively for auth session management, NOT for direct table queries); `createBrowserClient()` for auth state in client components; `createServerClient(cookies)` for SSR session refresh in server components and route handlers using `@supabase/ssr`
